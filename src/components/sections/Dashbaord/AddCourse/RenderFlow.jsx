      ...
      <div className="mb-12">
        ...
        <div className="relative mb-6 flex w-full justify-center">
          {steps.map((item, index) => (
            <div key={item.id} className="flex flex-col items-center min-w-[140px]">
              <div className="flex items-center">
                <div className="flex flex-col items-center">
                  <div
                    className={`relative grid aspect-square w-12 h-12 place-items-center rounded-full border-2 transition-all duration-300 ${step === item.id
                ...
                {index !== steps.length - 1 && (
                  <div className="flex items-center mx-4">
                    ...
                    >
                      {step > item.id && (
                        <div className="h-full w-full bg-gradient-to-r from-green-400 to-blue-400 animate-pulse"></div>
                      )}
                    </div>
                  </div>
                )}
              ...
              {/* Step label and underline (FIXED) */}
              <div className="flex flex-col items-center mt-2">
                ...
                {step === item.id && (
                  <div className="mt-2 h-0.5 w-8 bg-gradient-to-r from-blue-400 to-purple-400 rounded-full"></div>
                )}
              </div>
            </div>
          ))}
