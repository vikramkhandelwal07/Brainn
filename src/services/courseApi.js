...
import { setLoading } from "../slices/userProfileSlice";
...
export const updateSubSection = async (data, token) => {
  ...
  try {
    const response = await apiConnector("POST", UPDATE_SUBSECTION_API, data, {
      "Content-Type": "multipart/form-data", // Added for potential file upload
      Authorization: `Bearer ${token}`,
    });
...
export const deleteSubSection = async (data, token) => {
  ...
  const toastId = toast.loading("Loading...");
  try {
    // Updated to use params in URL as per backend expectation
    const response = await apiConnector(
      "DELETE",
      `${DELETE_SUBSECTION_API}/${data.subSectionId}/${data.sectionId}`,
      null,
      {
        Authorization: `Bearer ${token}`,
      }
